...
// UnmarshalJSON implements custom JSON unmarshalling for BifrostChatResponse.
// It normalizes vendor-specific reasoning fields that appear at the root level of the response/chunk
// into the first choice's delta or message.
func (cr *BifrostChatResponse) UnmarshalJSON(data []byte) error {
	type Alias BifrostChatResponse
	var aux struct {
		Alias
	...
	*cr = BifrostChatResponse(aux.Alias)

	var topReasoning *string
	if aux.ReasoningContent != nil {
		topReasoning = aux.ReasoningContent
	} else if aux.ReasoningAlias != nil {
		topReasoning = aux.ReasoningAlias
	} else if aux.Thought != nil {
		topReasoning = aux.Thought
	} else if aux.Thoughts != nil {
		topReasoning = aux.Thoughts
	} else if aux.Think != nil {
		topReasoning = aux.Think
	} else if aux.Thinking != nil {
		topReasoning = aux.Thinking
	} else if aux.ReasoningText != nil {
		topReasoning = aux.ReasoningText
	}

	if topReasoning != nil && *topReasoning != "" {
		if len(cr.Choices) == 0 {
			cr.Choices = []BifrostResponseChoice{
				{
					Index: 0,
					ChatStreamResponseChoice: &ChatStreamResponseChoice{
						Delta: &ChatStreamResponseChoiceDelta{Reasoning: topReasoning},
					},
				},
			}
		} else {
			...
			if choice.ChatStreamResponseChoice != nil {
				if choice.ChatStreamResponseChoice.Delta == nil {
					choice.ChatStreamResponseChoice.Delta = &ChatStreamResponseChoiceDelta{Reasoning: topReasoning}
				} else if choice.ChatStreamResponseChoice.Delta.Reasoning == nil {
					choice.ChatStreamResponseChoice.Delta.Reasoning = topReasoning
				}
			} else if choice.ChatNonStreamResponseChoice != nil && choice.ChatNonStreamResponseChoice.Message != nil {
				if choice.ChatNonStreamResponseChoice.Message.ChatAssistantMessage == nil {
					choice.ChatNonStreamResponseChoice.Message.ChatAssistantMessage = &ChatAssistantMessage{}
				}
				if choice.ChatNonStreamResponseChoice.Message.ChatAssistantMessage.Reasoning == nil {
					choice.ChatNonStreamResponseChoice.Message.ChatAssistantMessage.Reasoning = topReasoning
				}
			}
		}
	}
...
func (cm *ChatMessage) UnmarshalJSON(data []byte) error {
	...
	// If content contains inline <think>...</think> tags and Reasoning is not yet set, extract them.
	if cm.Role == ChatMessageRoleAssistant && cm.Content != nil && cm.Content.ContentStr != nil {
		content := *cm.Content.ContentStr
		start := strings.Index(content, "<think>")
		end := strings.Index(content, "</think>")
		if start != -1 && end != -1 && end > start {
			thinkText := strings.TrimSpace(content[start+len("<think>") : end])
			cleanText := strings.TrimSpace(content[:start] + content[end+len("</think>"):])
			if cm.ChatAssistantMessage == nil {
				cm.ChatAssistantMessage = &ChatAssistantMessage{}
			}
			if cm.ChatAssistantMessage.Reasoning == nil {
				cm.ChatAssistantMessage.Reasoning = &thinkText
			}
			cm.Content.ContentStr = &cleanText
		}
	}
...
func (cm *ChatAssistantMessage) UnmarshalJSON(data []byte) error {
	...
	// Auxiliary struct to capture various upstream vendor reasoning field names
	var aux struct {
		Alias
		ReasoningContent *string `json:"reasoning_content,omitempty"` // DeepSeek, xAI, SiliconFlow, vLLM
		ReasoningAlias   *string `json:"reasoning,omitempty"`         // OpenRouter
		Thought          *string `json:"thought,omitempty"`           // DashScope/Qwen, Ollama, Gemini
		Thoughts         *string `json:"thoughts,omitempty"`          // Baidu Qianfan
		Think            *string `json:"think,omitempty"`             // Vendor think field
		Thinking         *string `json:"thinking,omitempty"`          // Cloudflare, custom proxies
		ReasoningText    *string `json:"reasoning_text,omitempty"`    // Alternative spelling
	}
	...
	*cm = ChatAssistantMessage(aux.Alias)
	...
	// Map any vendor reasoning fields to Bifrost's Reasoning field
	if cm.Reasoning == nil {
		if aux.ReasoningContent != nil {
			cm.Reasoning = aux.ReasoningContent
		} else if aux.ReasoningAlias != nil {
			cm.Reasoning = aux.ReasoningAlias
		} else if aux.Thought != nil {
			cm.Reasoning = aux.Thought
		} else if aux.Thoughts != nil {
			cm.Reasoning = aux.Thoughts
		} else if aux.Think != nil {
			cm.Reasoning = aux.Think
		} else if aux.Thinking != nil {
			cm.Reasoning = aux.Thinking
		} else if aux.ReasoningText != nil {
			cm.Reasoning = aux.ReasoningText
		}
	}
...
// UnmarshalJSON implements custom JSON unmarshalling for BifrostResponseChoice.
// It normalizes vendor-specific reasoning fields that appear directly on the choice object
// (sibling to delta or message) into the appropriate delta or message reasoning field.
func (c *BifrostResponseChoice) UnmarshalJSON(data []byte) error {
	...
	*c = BifrostResponseChoice(aux.Alias)

	var choiceReasoning *string
	if aux.ReasoningContent != nil {
		choiceReasoning = aux.ReasoningContent
	} else if aux.ReasoningAlias != nil {
		choiceReasoning = aux.ReasoningAlias
	} else if aux.Thought != nil {
		choiceReasoning = aux.Thought
	} else if aux.Thoughts != nil {
		choiceReasoning = aux.Thoughts
	} else if aux.Think != nil {
		choiceReasoning = aux.Think
	} else if aux.Thinking != nil {
		choiceReasoning = aux.Thinking
	} else if aux.ReasoningText != nil {
		choiceReasoning = aux.ReasoningText
	}

	if choiceReasoning != nil && *choiceReasoning != "" {
		if c.ChatStreamResponseChoice != nil {
			if c.ChatStreamResponseChoice.Delta == nil {
				c.ChatStreamResponseChoice.Delta = &ChatStreamResponseChoiceDelta{Reasoning: choiceReasoning}
			} else if c.ChatStreamResponseChoice.Delta.Reasoning == nil {
				c.ChatStreamResponseChoice.Delta.Reasoning = choiceReasoning
			}
		} else if c.ChatNonStreamResponseChoice != nil && c.ChatNonStreamResponseChoice.Message != nil {
			if c.ChatNonStreamResponseChoice.Message.ChatAssistantMessage == nil {
				c.ChatNonStreamResponseChoice.Message.ChatAssistantMessage = &ChatAssistantMessage{}
			}
			if c.ChatNonStreamResponseChoice.Message.ChatAssistantMessage.Reasoning == nil {
				c.ChatNonStreamResponseChoice.Message.ChatAssistantMessage.Reasoning = choiceReasoning
			}
		} else if c.ChatNonStreamResponseChoice == nil && c.TextCompletionResponseChoice == nil {
			c.ChatStreamResponseChoice = &ChatStreamResponseChoice{
				Delta: &ChatStreamResponseChoiceDelta{Reasoning: choiceReasoning},
			}
		}
	}
	...
	*d = ChatStreamResponseChoiceDelta(aux.Alias)
	...
	if d.Reasoning == nil {
		if aux.ReasoningContent != nil {
			...
			d.Reasoning = aux.ReasoningAlias
		} else if aux.Thought != nil {
			d.Reasoning = aux.Thought
		} else if aux.Thoughts != nil {
			d.Reasoning = aux.Thoughts
		} else if aux.Think != nil {
			d.Reasoning = aux.Think
		} else if aux.Thinking != nil {
			d.Reasoning = aux.Thinking
		} else if aux.ReasoningText != nil {
			d.Reasoning = aux.ReasoningText
		}
	}
