...
func TestToBifrostResponsesStreamResponse_LifecycleEventsAreValidResponses(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	role := string(ChatMessageRoleAssistant)
	events := (&BifrostChatResponse{
		ID:    "chatcmpl-test",
		Model: "test-model",
		Choices: []BifrostResponseChoice{{
			ChatStreamResponseChoice: &ChatStreamResponseChoice{
	...
	for _, event := range events[:2] {
		if event.Response == nil {
			t.Fatalf("%s must include a response", event.Type)
		}
		if event.Response.Status == nil || *event.Response.Status != "in_progress" {
			t.Fatalf("%s status = %v, want in_progress", event.Type, event.Response.Status)
		}
		if event.Response.Object != "response" || event.Response.Model != "test-model" {
			t.Fatalf("%s response metadata = object:%q model:%q", event.Type, event.Response.Object, event.Response.Model)
		}
		if event.Response.Output == nil {
			t.Fatalf("%s output must be an empty array, not null", event.Type)
		}
	}
}

func TestToBifrostResponsesStreamResponse_EmitsStatusForEveryResponseAndItem(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)
	...
	makeChunk := func(role *string, content *string, finishReason *string) *BifrostChatResponse {
		return &BifrostChatResponse{
			...
			Choices: []BifrostResponseChoice{{
				FinishReason: finishReason,
				ChatStreamResponseChoice: &ChatStreamResponseChoice{
					Delta: &ChatStreamResponseChoiceDelta{Role: role, Content: content},
				},
			}},
		}
	}
	...
	events = append(events, makeChunk(&role, nil, nil).ToBifrostResponsesStreamResponse(state)...)
	events = append(events, makeChunk(nil, &content, nil).ToBifrostResponsesStreamResponse(state)...)
	events = append(events, makeChunk(nil, nil, &stop).ToBifrostResponsesStreamResponse(state)...)

	for _, event := range events {
		if event.Response != nil && event.Response.Status == nil {
			t.Fatalf("%s response is missing status", event.Type)
		}
		if event.Item != nil && event.Item.Status == nil {
			t.Fatalf("%s item is missing status", event.Type)
		}
	}
}
...
func TestToBifrostResponsesStreamResponse_ReasoningSummaryLifecycle(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	chunks := []*BifrostChatResponse{
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Reasoning: Ptr("Let me ")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Reasoning: Ptr("think.")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Content: Ptr("Answer")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{}, Ptr("stop")),
	}
	...
	var hasOutputItemAdded, hasSummaryPartAdded, hasSummaryTextDelta bool
	var hasSummaryTextDone, hasSummaryPartDone, hasOutputItemDone, hasCompleted bool
	...
	for _, e := range events {
		...
		case ResponsesStreamResponseTypeOutputItemAdded:
			if e.Item != nil && e.Item.Type != nil && *e.Item.Type == ResponsesMessageTypeReasoning {
				...
				if e.Item.ResponsesReasoning == nil || e.Item.ResponsesReasoning.Summary == nil {
					t.Fatalf("expected summary array in reasoning item.added, got %+v", e.Item.ResponsesReasoning)
				}
			}
		case ResponsesStreamResponseTypeReasoningSummaryPartAdded:
			hasSummaryPartAdded = true
			if e.SummaryIndex == nil || *e.SummaryIndex != 0 {
				t.Fatalf("expected summary_index 0 in reasoning_summary_part.added, got %v", e.SummaryIndex)
			}
			if e.Part == nil || e.Part.Type != "summary_text" {
				t.Fatalf("expected summary_text part in reasoning_summary_part.added, got %+v", e.Part)
			}
		case ResponsesStreamResponseTypeReasoningSummaryTextDelta:
			hasSummaryTextDelta = true
			if e.SummaryIndex == nil || *e.SummaryIndex != 0 {
				t.Fatalf("expected summary_index 0 in reasoning_summary_text.delta, got %v", e.SummaryIndex)
			}
		case ResponsesStreamResponseTypeReasoningSummaryTextDone:
			hasSummaryTextDone = true
			if e.SummaryIndex == nil || *e.SummaryIndex != 0 {
				t.Fatalf("expected summary_index 0 in reasoning_summary_text.done, got %v", e.SummaryIndex)
			}
			if e.Text == nil || *e.Text != "Let me think." {
				t.Fatalf("expected full reasoning text in reasoning_summary_text.done, got %v", e.Text)
			}
		case ResponsesStreamResponseTypeReasoningSummaryPartDone:
			hasSummaryPartDone = true
			if e.SummaryIndex == nil || *e.SummaryIndex != 0 {
				t.Fatalf("expected summary_index 0 in reasoning_summary_part.done, got %v", e.SummaryIndex)
			}
			if e.Part == nil || e.Part.Type != "summary_text" || e.Part.Text == nil || *e.Part.Text != "Let me think." {
				t.Fatalf("expected full reasoning in reasoning_summary_part.done, got %+v", e.Part)
			}
		case ResponsesStreamResponseTypeOutputItemDone:
			if e.Item != nil && e.Item.Type != nil && *e.Item.Type == ResponsesMessageTypeReasoning {
				hasOutputItemDone = true
				if e.Item.ResponsesReasoning == nil || len(e.Item.ResponsesReasoning.Summary) == 0 {
					t.Fatalf("expected populated summary in reasoning output_item.done, got %+v", e.Item.ResponsesReasoning)
				}
				if e.Item.ResponsesReasoning.Summary[0].Text != "Let me think." {
					t.Fatalf("expected 'Let me think.' in summary, got %q", e.Item.ResponsesReasoning.Summary[0].Text)
				}
			}
		case ResponsesStreamResponseTypeCompleted:
			hasCompleted = true
			if e.Response == nil || len(e.Response.Output) == 0 {
				t.Fatalf("expected output in response.completed")
			}
			reasoningItem := e.Response.Output[0]
			if reasoningItem.Type == nil || *reasoningItem.Type != ResponsesMessageTypeReasoning {
				t.Fatalf("expected first output item to be reasoning, got %v", reasoningItem.Type)
			}
			if reasoningItem.ResponsesReasoning == nil || len(reasoningItem.ResponsesReasoning.Summary) == 0 {
				t.Fatalf("expected populated summary in completed reasoning output, got %+v", reasoningItem.ResponsesReasoning)
			}
			if reasoningItem.ResponsesReasoning.Summary[0].Text != "Let me think." {
				t.Fatalf("expected 'Let me think.' in completed summary, got %q", reasoningItem.ResponsesReasoning.Summary[0].Text)
			}
		}
	}

	if !hasOutputItemAdded {
		t.Fatal("missing reasoning output_item.added")
	}
	if !hasSummaryPartAdded {
		t.Fatal("missing reasoning_summary_part.added")
	}
	if !hasSummaryTextDelta {
		t.Fatal("missing reasoning_summary_text.delta")
	}
	if !hasSummaryTextDone {
		t.Fatal("missing reasoning_summary_text.done")
	}
	if !hasSummaryPartDone {
		t.Fatal("missing reasoning_summary_part.done")
	}
...
func TestToBifrostResponsesStreamResponse_InBandThinkTagExtraction(t *testing.T) {
	state := AcquireChatToResponsesStreamState()
	defer ReleaseChatToResponsesStreamState(state)

	chunks := []*BifrostChatResponse{
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Content: Ptr("<think>Pondering")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Content: Ptr(" deeply</think>Here is ")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{Content: Ptr("the answer")}, nil),
		streamChunk("c1", &ChatStreamResponseChoiceDelta{}, Ptr("stop")),
	}
	...
	if fullReasoning != "Pondering deeply" {
		t.Fatalf("expected extracted reasoning 'Pondering deeply', got %q", fullReasoning)
	}
	if fullText != "Here is the answer" {
		t.Fatalf("expected extracted text 'Here is the answer', got %q", fullText)
	}
}
