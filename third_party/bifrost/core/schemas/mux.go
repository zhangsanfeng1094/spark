	...
	if cm.Role == ChatMessageRoleAssistant && cm.ChatAssistantMessage != nil {
		...
		if len(am.ReasoningDetails) > 0 {
			...
			if len(contentBlocks) > 0 || len(summaries) > 0 || encryptedContent != nil {
				...
				if len(summaries) == 0 && len(contentBlocks) > 0 {
...
type ChatToResponsesStreamState struct {
	...
	HasEmittedCompleted   bool              // Whether we've emitted response.completed
	...
	ReasoningPartAdded    bool              // Whether response.reasoning_summary_part.added has been emitted
	...
	InThinkTag            bool              // Whether currently inside in-band <think> block
	ThinkTagBuffer        string            // Buffered partial tag prefix
...
// extractStreamingThink parses in-band <think>...</think> tags from content deltas.
// It returns reasoning text (if any) and normal content text (if any).
func (state *ChatToResponsesStreamState) extractStreamingThink(content string) (reasoningText string, normalText string) {
	if content == "" && state.ThinkTagBuffer == "" {
		return "", ""
	}
	raw := state.ThinkTagBuffer + content
	state.ThinkTagBuffer = ""

	if !state.InThinkTag {
		if idx := strings.Index(raw, "<think>"); idx >= 0 {
			pre := raw[:idx]
			after := raw[idx+len("<think>"):]
			state.InThinkTag = true
			r, n := state.extractStreamingThink(after)
			if pre != "" {
				if n != "" {
					n = pre + n
				} else {
		...
		thinkTag := "<think>"
		for i := len(thinkTag) - 1; i >= 1; i-- {
			prefix := thinkTag[:i]
			if strings.HasSuffix(raw, prefix) {
				state.ThinkTagBuffer = prefix
				normalText = strings.TrimSuffix(raw, prefix)
				return "", normalText
			}
		}
		return "", raw
	...
	if idx := strings.Index(raw, "</think>"); idx >= 0 {
		reasoning := raw[:idx]
		post := raw[idx+len("</think>"):]
		state.InThinkTag = false
		r, n := state.extractStreamingThink(post)
		if reasoning != "" {
			if r != "" {
				reasoning += r
			}
	...
	for i := len(endTag) - 1; i >= 1; i-- {
		...
		if strings.HasSuffix(raw, prefix) {
			state.ThinkTagBuffer = prefix
			reasoningText = strings.TrimSuffix(raw, prefix)
...
func (cr *BifrostChatResponse) ToBifrostResponsesStreamResponse(state *ChatToResponsesStreamState) []*BifrostResponsesStreamResponse {
	...
	// Store message ID and model from first chunk if present
	...
	if !state.HasEmittedCreated {
		if state.MessageID == nil {
			if cr.ID != "" {
				...
				genID := fmt.Sprintf("resp_%d", time.Now().UnixNano())
				state.MessageID = &genID
			}
		}
		model := ""
		if state.Model != nil {
	...
	// If reasoning is not explicitly provided, check if delta.Content contains in-band <think> tags.
	if (delta.Reasoning == nil || *delta.Reasoning == "") && delta.Content != nil && *delta.Content != "" {
		reasoningText, normalText := state.extractStreamingThink(*delta.Content)
		if reasoningText != "" {
			delta.Reasoning = &reasoningText
		}
		if normalText != "" {
			delta.Content = &normalText
		} else {
			delta.Content = nil
		}
	}
	...
	closeReasoningItem := func() {
		...
		if state.ReasoningPartAdded {
			...
			donePart := &ResponsesMessageContentBlock{
				Type: ResponsesMessageContentBlockType(ResponsesReasoningContentBlockTypeSummaryText),
				Text: &reasoningText,
			}
			responses = append(responses, &BifrostResponsesStreamResponse{
	...
	if hasReasoning && !state.ReasoningItemClosed {
		if !state.ReasoningItemAdded {
			...
			emptyText := ""
			summaryPart := &ResponsesMessageContentBlock{
				Type: ResponsesMessageContentBlockType(ResponsesReasoningContentBlockTypeSummaryText),
				Text: &emptyText,
			}
			responses = append(responses, &BifrostResponsesStreamResponse{
...
// FinalizeStream emits closing events for any unclosed items and the terminal
// response.completed event if it has not been emitted yet.
func (state *ChatToResponsesStreamState) FinalizeStream(usage *BifrostLLMUsage) []*BifrostResponsesStreamResponse {
	if state == nil || !state.HasEmittedCreated || state.HasEmittedCompleted {
		return nil
	}
	...
	var respUsage *ResponsesResponseUsage
	if usage != nil {
		respUsage = usage.ToResponsesResponseUsage()
	}
