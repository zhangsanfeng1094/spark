...
func bedrockS3LocationUnsupportedError(model, kind, uri, alternative string) error {
	return providerUtils.InvalidRequestErrorf("model %q does not support s3:// references for %s %q; provide %s instead", model, kind, uri, alternative)
}
