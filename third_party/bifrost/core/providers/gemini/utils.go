...
// convertChatContentBlockToGeminiPart converts a single ChatContentBlock (text, image, file, audio)
// into a Gemini Part, ensuring proper data sanitization, base64 validation, and MIME detection.
func convertChatContentBlockToGeminiPart(block schemas.ChatContentBlock, allowedImageURLSchemes ...string) (*Part, error) {
	if block.Text != nil && *block.Text != "" {
		return &Part{Text: *block.Text}, nil
	}
	...
	if block.ImageURLStruct != nil {
		...
		if urlInfo.Type == schemas.ImageContentTypeBase64 {
			if urlInfo.DataURLWithoutPrefix != nil {
				decodedData, err := decodeBase64StringToBytes(*urlInfo.DataURLWithoutPrefix)
				if err == nil && len(decodedData) >= 8 {
					return &Part{
						InlineData: &Blob{
							MIMEType: mimeType,
...
func convertBifrostMessagesToGemini(messages []schemas.ChatMessage, allowedImageURLSchemes ...string) ([]Content, *Content, error) {
	...
	// Track media parts extracted from tool responses to provide them in a follow-up user turn
	// so that Gemini's vision encoder is activated without violating function response constraints.
	var pendingToolMediaParts []*Part
	...
	flushPendingToolResponses := func() {
		...
		if len(pendingToolMediaParts) > 0 {
			label := "[images from tool execution — see attached]"
			synthParts := make([]*Part, 0, 1+len(pendingToolMediaParts))
			synthParts = append(synthParts, &Part{Text: label})
			synthParts = append(synthParts, pendingToolMediaParts...)
			contents = append(contents, Content{
				Parts: synthParts,
				Role:  "user",
			})
			pendingToolMediaParts = nil
		}
	}
	...
	for i, message := range messages {
		...
		if isToolResponse {
			...
			if message.Content != nil {
				...
				if message.Content.ContentStr != nil && *message.Content.ContentStr != "" {
					...
					// Fallback: try to extract text from content blocks and collect media parts
					var textParts []string
					for _, block := range message.Content.ContentBlocks {
						if block.Text != nil && *block.Text != "" {
							textParts = append(textParts, *block.Text)
						} else if block.ImageURLStruct != nil || block.File != nil {
							mediaPart, err := convertChatContentBlockToGeminiPart(block, allowedImageURLSchemes...)
							if err == nil && mediaPart != nil {
								pendingToolMediaParts = append(pendingToolMediaParts, mediaPart)
							}
						}
					}
					if len(textParts) > 0 {
						contentStr = strings.Join(textParts, "\n")
					} else if len(pendingToolMediaParts) > 0 {
						contentStr = "[see attached media]"
					}
				}
			}
		...
		if message.Content != nil {
			if message.Content.ContentStr != nil && *message.Content.ContentStr != "" {
				...
				for _, block := range message.Content.ContentBlocks {
					part, err := convertChatContentBlockToGeminiPart(block, allowedImageURLSchemes...)
					if err != nil {
						return nil, nil, err
					}
					if part != nil {
...
//
// If the input is missing padding or contains whitespace/newlines, decodeBase64StringToBytes
// cleans the input, normalizes URL-safe characters, and appends canonical RFC 4648 padding.
...
func decodeBase64StringToBytes(b64 string) ([]byte, error) {
	...
	standardBase64 := strings.ReplaceAll(strings.ReplaceAll(clean, "_", "/"), "-", "+")

	// Trim trailing padding to avoid double-padding, then recompute canonical padding
	unpadded := strings.TrimRight(standardBase64, "=")
	switch len(unpadded) % 4 {
	case 0:
		standardBase64 = unpadded
	case 2:
		standardBase64 = unpadded + "=="
	case 3:
		standardBase64 = unpadded + "="
	case 1:
		return nil, fmt.Errorf("invalid base64 length %d: trailing 6-bit quantum cannot form an 8-bit byte", len(unpadded))
	}
