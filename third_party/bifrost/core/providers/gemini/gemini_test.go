...
func TestToGeminiChatCompletionRequest_MultimodalToolOutput(t *testing.T) {
		...
		Input: []schemas.ChatMessage{
			{
				Role: schemas.ChatMessageRoleUser,
				Content: &schemas.ChatMessageContent{
					ContentStr: schemas.Ptr("Take a screenshot and check logs"),
				},
			},
			{
				Role: schemas.ChatMessageRoleAssistant,
			...
			// Tool 1 returns multimodal blocks (text + image)
			{
				Role: schemas.ChatMessageRoleTool,
				ChatToolMessage: &schemas.ChatToolMessage{
					ToolCallID: &callID1,
			...
			// Tool 2 returns text only
			{
				...
				Content: &schemas.ChatMessageContent{
					ContentStr: schemas.Ptr("System healthy"),
				},
			},
		},
	}

	out, err := gemini.ToGeminiChatCompletionRequest(nil, bReq)
	require.NoError(t, err)
	require.NotNil(t, out)

	// Verification:
	// Contents[0]: User prompt
	// Contents[1]: Model assistant tool calls
	// Contents[2]: Contiguous FunctionResponses (both call_1 and call_2 together!)
	// Contents[3]: Follow-up user turn carrying the image InlineData Blob
	require.Len(t, out.Contents, 4)

	// Verify FunctionResponses turn
	fnContent := out.Contents[2]
	assert.Equal(t, "user", fnContent.Role)
	require.Len(t, fnContent.Parts, 2, "Parallel function responses must remain contiguous in a single turn")
	assert.Equal(t, "call_1", fnContent.Parts[0].FunctionResponse.ID)
	assert.Equal(t, "call_2", fnContent.Parts[1].FunctionResponse.ID)

	// Verify Synthesized follow-up user turn with vision image
	imgContent := out.Contents[3]
	assert.Equal(t, "user", imgContent.Role)
	require.Len(t, imgContent.Parts, 2)
	assert.Contains(t, imgContent.Parts[0].Text, "images from tool")
	require.NotNil(t, imgContent.Parts[1].InlineData, "Media part must be populated as InlineData")
	assert.Equal(t, "image/png", imgContent.Parts[1].InlineData.MIMEType)
	assert.NotEmpty(t, imgContent.Parts[1].InlineData.Data)
}
