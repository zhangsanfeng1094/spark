...
func TestToOpenAIChatRequest_MultimodalToolOutput(t *testing.T) {
	pngURL := "https://example.com/chart.png"
	textOutput := "Generated chart"
	callID := "call_screenshot"
	callType := "function"
	funcName := "take_screenshot"
	...
	out := ToOpenAIChatRequest(nil, bReq)
	require.NotNil(t, out)
	// Expect: Assistant (1) + Tool with string content (1) + Follow-up user with image (1) = 3
	require.Len(t, out.Messages, 3)

	toolMsg := out.Messages[1]
	assert.Equal(t, schemas.ChatMessageRoleTool, toolMsg.Role)
	require.NotNil(t, toolMsg.Content.ContentStr, "OpenAI tool message content must be string")
	assert.Equal(t, "Generated chart", *toolMsg.Content.ContentStr)

	synthUserMsg := out.Messages[2]
	assert.Equal(t, schemas.ChatMessageRoleUser, synthUserMsg.Role)
	require.NotNil(t, synthUserMsg.Content.ContentBlocks)
	require.Len(t, synthUserMsg.Content.ContentBlocks, 2)
	assert.Contains(t, *synthUserMsg.Content.ContentBlocks[0].Text, "images from tool")
	assert.Equal(t, schemas.ChatContentBlockTypeImage, synthUserMsg.Content.ContentBlocks[1].Type)
	assert.Equal(t, pngURL, synthUserMsg.Content.ContentBlocks[1].ImageURLStruct.URL)
}
