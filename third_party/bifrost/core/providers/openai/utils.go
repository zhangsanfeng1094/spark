...
func ConvertBifrostMessagesToOpenAIMessages(messages []schemas.ChatMessage) []OpenAIMessage {
	...
	var pendingToolMediaBlocks []schemas.ChatContentBlock

	flushPendingToolMedia := func() {
		if len(pendingToolMediaBlocks) == 0 {
			return
		}
		label := "[images from tool execution — see attached]"
		synthBlocks := make([]schemas.ChatContentBlock, 0, 1+len(pendingToolMediaBlocks))
		synthBlocks = append(synthBlocks, schemas.ChatContentBlock{
			Type: schemas.ChatContentBlockTypeText,
			Text: &label,
		})
		synthBlocks = append(synthBlocks, pendingToolMediaBlocks...)
		openaiMessages = append(openaiMessages, OpenAIMessage{
			Role: schemas.ChatMessageRoleUser,
			Content: &schemas.ChatMessageContent{
				ContentBlocks: synthBlocks,
			},
		})
		pendingToolMediaBlocks = nil
	}

	for i, message := range messages {
		isTool := message.Role == schemas.ChatMessageRoleTool && message.ChatToolMessage != nil
		if !isTool && len(pendingToolMediaBlocks) > 0 {
			flushPendingToolMedia()
		}

		targetContent := message.Content
		if isTool && message.Content != nil && message.Content.ContentBlocks != nil {
			// OpenAI tool role requires string content. Extract text and hoist any images.
			var textParts []string
			for _, block := range message.Content.ContentBlocks {
				if block.Text != nil && *block.Text != "" {
					textParts = append(textParts, *block.Text)
				} else if block.ImageURLStruct != nil || block.File != nil {
					pendingToolMediaBlocks = append(pendingToolMediaBlocks, block)
				}
			}
			text := strings.Join(textParts, "\n")
			if text == "" && len(pendingToolMediaBlocks) > 0 {
				text = "[see attached media]"
			}
			targetContent = &schemas.ChatMessageContent{ContentStr: &text}
		}
		...
		if msg.ChatToolMessage != nil && msg.ChatToolMessage.ToolCallID != nil &&
			len(*msg.ChatToolMessage.ToolCallID) > MaxToolCallIDLength {
			if stripped := utils.StripThoughtSignature(*msg.ChatToolMessage.ToolCallID); stripped != *msg.ChatToolMessage.ToolCallID {
				toolMsgCopy := *msg.ChatToolMessage
				toolMsgCopy.ToolCallID = &stripped
				msg.ChatToolMessage = &toolMsgCopy
			}
		}
		...
		if message.ChatAssistantMessage != nil {
			...
			msg.OpenAIChatAssistantMessage = &OpenAIChatAssistantMessage{
		...
		if i == len(messages)-1 && len(pendingToolMediaBlocks) > 0 {
			flushPendingToolMedia()
		}
	}
