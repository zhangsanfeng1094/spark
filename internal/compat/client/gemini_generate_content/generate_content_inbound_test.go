...
func TestGenerateContentInboundMapsContentsAndConfig(t *testing.T) {
	...
	if req.Source != ir.ProtocolGeminiGenerateContent || req.Model != "gemini-2.5-flash" {
		t.Fatalf("request basics mismatch: %#v", req)
	}
	if len(req.Messages) != 4 {
		t.Fatalf("messages mismatch: %#v", req.Messages)
	}
	if req.Messages[0].Role != ir.RoleSystem || req.Messages[0].Text() != "be concise" {
		t.Fatalf("system message mismatch: %#v", req.Messages[0])
	}
	if req.Messages[2].Role != ir.RoleAssistant || req.Messages[2].Text() != "hi" {
	...
	if req.Messages[3].Role != ir.RoleTool || len(req.Messages[3].Content) != 1 {
	...
	if req.Generation.Reasoning.IncludeThoughts == nil || !*req.Generation.Reasoning.IncludeThoughts {
		t.Fatalf("include thoughts mismatch: %#v", req.Generation.Reasoning)
	}
	if req.Generation.Reasoning.BudgetTokens == nil || *req.Generation.Reasoning.BudgetTokens != 256 {
