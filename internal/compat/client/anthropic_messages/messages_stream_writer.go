...
func WriteMessagesStream(writer io.Writer, reader io.Reader, requestedModel string, flush func()) MessagesStreamResult {
	...
	for scanner.Scan() {
		...
		for _, event := range openai_chat_target.ChatStreamEvents(chunk) {
			...
			case ir.StreamEventResponseDone:
		...
		fallback := openai_chat_target.ChatResponse(chunk)
	...
	if !state.messageStarted {
		...
		resp := openai_chat_target.ChatResponse(finalChunk)
...
func (s *messagesStreamState) observeUsage(usage ir.Usage) {
...
func (s *messagesStreamState) writeToolCallDelta(block ir.ContentBlock) {
	call := block.ToolCall
	if call == nil {
		return
	}
...
func (s *messagesStreamState) addFullResponseToolCalls(blocks []ir.ContentBlock) {
	for idx, tc := range responseToolCalls(blocks) {
		s.writeToolCallDelta(ir.ContentBlock{
...
func anthropicStreamStopReason(reason ir.StopReason) string {
	switch reason {
...
func responseReasoningText(blocks []ir.ContentBlock) string {
	parts := make([]string, 0, len(blocks))
	for _, block := range blocks {
		if block.Type == ir.BlockReasoning && block.Reasoning != nil && block.Reasoning.Text != "" {
...
func responseToolCallIDs(blocks []ir.ContentBlock) []string {
	ids := make([]string, 0, len(blocks))
	for _, block := range blocks {
		if block.Type == ir.BlockToolCall && block.ToolCall != nil && block.ToolCall.ID != "" {
