...
func ResponsesClientResponse(resp ir.Response) map[string]any {
	id := resp.ID
	if id == "" {
...
func ResponsesUsage(usage ir.Usage) (map[string]any, bool) {
	raw := usage.Raw
...
func ResponsesUsageFromChatPayload(payload map[string]any) (map[string]any, bool) {
	...
	return ResponsesUsage(ir.Usage{Raw: usage})
}
...
func responseOutputText(blocks []ir.ContentBlock) string {
...
func responseReasoningOutputText(blocks []ir.ContentBlock) string {
	parts := make([]string, 0, len(blocks))
	for _, block := range blocks {
