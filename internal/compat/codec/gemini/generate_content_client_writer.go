...
func GenerateContentClientResponse(resp compatir.Response) map[string]any {
...
func geminiResponseParts(blocks []compatir.ContentBlock) []map[string]any {
...
func geminiUsageMetadata(usage compatir.Usage) (map[string]any, bool) {
	raw := usage.Raw
...
func geminiFinishReason(reason compatir.StopReason) string {
	switch reason {
	case compatir.StopReasonMaxTokens:
		return "MAX_TOKENS"
	case compatir.StopReasonContentFilter:
		return "SAFETY"
	case compatir.StopReasonError:
		return "OTHER"
	default:
		return "STOP"
	}
